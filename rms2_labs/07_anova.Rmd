...
`r qbegin(2)`
Create a frequency table showing the number of people for each type of background music.
`r qend()`
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
...
- $i = 1, 2, 3$ counts the group or music type
- $j = 1, 2, ..., 120$ counts the observation within a group
- $y_{i,j}$ is the observed spending for subject $j$ from group $i$
...
The model has now become _not identifiable._ We cannot distinguish any more whether the group mean is due to a group effect or not.
...
|:---------------:|:---------------------------------:|:-------------------------------------------:|:-----------------:|
|   Sum to zero   | $\beta_1 + \beta_2 + \beta_3 = 0$ | $\beta_0 = \frac{\mu_1 + \mu_2 + \mu_3}{3}$ |    `contr.sum`    |
...
- $\beta_0$ is interpreted as the overall or global mean --- that is, the population mean when all the groups are combined;
- $\beta_i$ is interpreted as the effect of group $i$ --- that is, the difference between the population mean for group $i$, $\mu_i$, and the global population mean, $\mu$.
...
$$
\beta_i = \mu_i - \beta_0 = \mu_i - \left( \frac{\mu_1 + \mu_2 + \mu_3}{3} \right)
$$
...
Under the constraint $\beta_1 = 0$, meaning that the first group is the reference group,
